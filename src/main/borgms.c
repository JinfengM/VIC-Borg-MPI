...
void BORG_Worker_run(int argc, char* argv[],int rank) {
	int rc;
	...
	/* process jobs */
	while (1) {
		fprintf(stderr,"I am working on host:%s with rank ID : %d\n",hostname,rank);
		BORG_Timer_start("wait");
		...
		if (rc != MPI_SUCCESS) {
			...
			problem->function(input, output, ncons == 0 ? NULL : output+nobjs,argc,argv,rank);
...
BORG_Archive BORG_Algorithm_ms_run(BORG_Problem _problem,int argc, char* argv[],int rank) {
	...
	if (worldRank == 0) {
		...
		BORG_Timer_open();
		BORG_Debug("Starting!\n");
		BORG_Worker_run(argc,argv,rank);
		BORG_Debug("Exiting!\n");
	}
