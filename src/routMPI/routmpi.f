...
c     no changes after here
      ...
      CHARACTER*72 INPATH, OUTPATH, TEMP
      CHARACTER*8  SRANK
...
c     read input path and precision of VIC filenames
      ...
      TEMP=TRIM(INPATH)
      write(SRANK,"(I4)")rank
	  INPATH=TRIM(INPATH)//trim(adjustl(SRANK))//"/fluxes_"
	  PRINT*, 'INPATH ', INPATH
      READ(1,*)DPREC
c     output pathname
      READ(1,'(/A)')OUTPATH
      OUTPATH=TRIM(adjustl(OUTPATH))//trim(adjustl(SRANK))//"/"
...
C     Loop over required stations
 ...
 100  CONTINUE
     ...
     &        NR, NAME, PI, PJ
         PRINT*, 'Routing station NAME: ', NAME,'UH_STRING:',UH_STRING
...
c     add XC,YC,SIZE TO CALCULATE geo location
     ...
     &        NO_OF_BOX,CATCHIJ,PMAX,IROW,ICOL,XC,YC,SIZE,NAME5)
      ...
      print*, 'reading grid_UH...',FILENAME
         CALL READ_GRID_UH
     ...
     &        UH_STRING,NAME5,TEMP,SRANK)        !new, AW:  added uh_string
