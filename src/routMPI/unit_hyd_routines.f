...
c     Solve for 
     ...
     &  UH_S, UH_STRING, NAME5,TEMP,SRANK)
...
c       默认存放在汇流执行程序所在目录        
c       open(98, file = NAME5//'.uh_s', STATUS='UNKNOWN') 
c       现在另存在结果文件中TRIM(INPATH)//trim(adjustl(SRANK))//
        TEMP=TRIM(TEMP)//trim(adjustl(SRANK))//"/"//NAME5//'.uh_s'
        print*,TEMP
        open(98, file =TEMP, STATUS='UNKNOWN')
        CLOSE(98,STATUS='DELETE')		
        open(98, file = TEMP, status='new') 		
